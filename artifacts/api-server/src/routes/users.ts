...
router.patch("/users/me", requireAuth, async (req, res) => {
  ...
  try {
    const [user] = await db.update(usersTable).set({ resumeName }).where(eq(usersTable.id, uid)).returning();
    res.json(user);
  } catch (err: any) {
...
router.get("/admin/setup", requireAuth, async (req, res) => {
  ...
  try {
    ...
    const [user] = await db.update(usersTable).set({ role: "admin" }).where(eq(usersTable.id, uid)).returning();
...
router.post("/users/upgrade-request", requireAuth, async (req, res) => {
  ...
  try {
    ...
    }).where(eq(usersTable.id, uid)).returning();
    res.json(updated);
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
